package com.xcyoung.cyberframe.widget
...
class BubbleLayout(context: Context, attrs: AttributeSet?) : FrameLayout(context, attrs) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        //阴影颜色
        val shadowColor = ta.getColor(R.styleable.BubbleLayout_shadow_color,
                Color.parseColor("#999999"))
        val defShadowSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX,
                4f, resources.displayMetrics).toInt()
        //阴影尺寸
    ...
    private fun drawLeftTriangle(canvas: Canvas) {
        ...
        mPath!!.moveTo(mDatumPoint!!.x.toFloat(), (mDatumPoint!!.y - triangularLength / 2).toFloat())
        mPath!!.lineTo((mDatumPoint!!.x - triangularLength / 2).toFloat(), mDatumPoint!!.y.toFloat())
        mPath!!.lineTo(mDatumPoint!!.x.toFloat(), (mDatumPoint!!.y + triangularLength / 2).toFloat())
        mPath!!.close()
    ...
    private fun drawTopTriangle(canvas: Canvas) {
        ...
        mPath!!.addRoundRect(mRect, mRadius.toFloat(), mRadius.toFloat(), Path.Direction.CCW)
        mPath!!.moveTo((mDatumPoint!!.x + triangularLength / 2).toFloat(), mDatumPoint!!.y.toFloat())
        mPath!!.lineTo(mDatumPoint!!.x.toFloat(), (mDatumPoint!!.y - triangularLength / 2).toFloat())
        mPath!!.lineTo((mDatumPoint!!.x - triangularLength / 2).toFloat(), mDatumPoint!!.y.toFloat())
        mPath!!.close()
        canvas.drawPath(mPath!!, mBorderPaint!!)
